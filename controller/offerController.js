...
const addOffer = async(req,res,next)=>{
    try {
        ...
        if(categoryCheck){
            return res.status(208).send({ success: false, message: "Category Already Exits" })
        } else {
            ...
            offerData.save()
            return res.status(200).send({ success: true , message: "Offer Added Successfully!"})
        }
    } catch (error) {
        console.log(error)
        next(new AppError('Sorry...Something went wrong', 500));
...
const editOffer = async(req,res,next)=>{
    try {
        ...
        if((offerCheck.length == 1 && offerId == offerCheck[0]._id) || offerCheck.length == 0){
            await offer.updateOne({_id:offerId},{$set:{ categoryId:categoryId, offerPercentage:offerPercentage, startDate:startDate, expiryDate:expiryDate }})
