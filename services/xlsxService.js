...
exports.generateExcel = async (orders, totalOrderAmount, discountAmount, start, end) => {
    ...
    if(start && end){
        ...
        if(start.length){
            ...
            worksheet.getCell('A2').value = `From: ${start.toISOString().split('T')[0]} To: ${end.toISOString().split('T')[0]}`;
        }
    ...
    worksheet.mergeCells('A4:B4'); worksheet.getCell('A4').value = 'S.No';
    worksheet.mergeCells('C4:D4'); worksheet.getCell('C4').value = 'Name';
    worksheet.mergeCells('E4:F4'); worksheet.getCell('E4').value = 'Delivery Date';
    worksheet.mergeCells('G4:H4'); worksheet.getCell('G4').value = 'No of Products';
    worksheet.mergeCells('I4:J4'); worksheet.getCell('I4').value = 'Total Cost';
    worksheet.mergeCells('K4:L4'); worksheet.getCell('K4').value = 'Payment Method';
    ...
    orders.forEach((order, index) => {
        ...
        worksheet.getCell(`E${rowIndex}`).value = order.deliveryDate ? order.deliveryDate.toISOString().split('T')[0] : 'N/A'
        worksheet.getCell(`G${rowIndex}`).value = order.products ? order.products.length : 0
        worksheet.getCell(`I${rowIndex}`).value = order.priceDetails && order.priceDetails.total ? order.priceDetails.total.toFixed(2) : '0.00';
    ...
    worksheet.getCell(`I${rowIndex}`).font = { bold: true, color: { argb: 'FF0000' } };
